...
def plot_video(joints,
    ...
    npy_folder = file_path + 'test_poses'  # to store numpy files for output poses (optional)
    npy_true_folder = file_path + 'gt_poses'
    if not os.path.exists(npy_folder):
        os.mkdir(npy_folder)
    if not os.path.exists(npy_true_folder):
        os.mkdir(npy_true_folder)
    FPS = (25 // skip_frames)
    video_file = file_path + "/{}.avi".format(video_name)
    ...
    for (j, frame_joints) in enumerate(joints):
        ...
        # frame_2d = frame_joints[:-1] / 3
        # pred_kpts.append(frame_2d)
        frame_joints_2d = np.reshape(frame_joints[:-1], (50, 3))
        # Draw the frame given 2D joints
        frame_2d = frame_joints_2d[:, :].flatten() / 3
        pred_kpts.append(frame_2d)

        ref_joints = references[j]
        ref_joints_2d = np.reshape(ref_joints[:-1], (50, 3))
        # Draw the frame given 2D joints
        ref_frame_2d = ref_joints_2d[:, :].flatten() / 3
        gt_kpts.append(ref_frame_2d)
    ...
    for (j, frame_joints) in enumerate(joints):
        ...
        # TODO - Remove the *3 if the joints weren't divided by 3 in data crea
        ...
        num_frames += 1
    # Release the video
    np.save(os.path.join(npy_folder, sequence_ID.split("/")[-1]), np.array(pred_kpts))
    np.save(os.path.join(npy_true_folder, sequence_ID.split("/")[-1]), np.array(gt_kpts))
    video.release()
...
def draw_frame_2D(frame, joints):
    ...
    # Increase the size and position of the joint
...
def alter_DTW_timing(pred_seq, ref_seq):
    ...
    new_pred_seq[:, :] = pred_seq[:ref_seq.shape[0], :]
    return pred_seq, ref_seq, d
