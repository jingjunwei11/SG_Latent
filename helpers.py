...
def calculate_dtw(references, hypotheses, trg_length):
    ...
    for i, ref in enumerate(references):
        ...
        # right_padding = max_length - length
        ...
        hyp = hypotheses[i]
        if left_padding + right_padding >= ref.shape[0]:
            ref_count = ref[:, :-1].cpu().numpy()
        else:
            ref_count = ref[left_padding:-right_padding, :-1].cpu().numpy()

        if left_padding + right_padding >= hyp.shape[0]:
            hyp_count = hyp[:, :-1].cpu().detach().numpy()
        else:
            hyp_count = hyp[left_padding:-right_padding, :-1].cpu().detach().numpy()
        # ref_count = ref[:-right_padding, :-1].cpu().numpy()
        ...
        # Cut down frames by to the max counter value, and chop off counter from joints
        # hyp_count = hyp[:-right_padding, :-1].cpu().detach().numpy()
...
def dtw_distance(x, y):
    ...
    dtw = torch.zeros((m, n), dtype=torch.float32, device=x.device)

    for i in range(m):
        for j in range(n):
            cost = pairwise_distance(x[:, i, :], y[:, j, :]).mean()
            if i == 0 and j == 0:
                dtw[i, j] = cost
            elif i == 0:
