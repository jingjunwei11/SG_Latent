...
from data_operate.vocab_dataset import build_vocab
...
def load_data(cfg: dict) -> (Dataset, Dataset, Optional[Dataset],
    ...
    skip_frames = data_cfg.get("skip_frames", 2)

    EOS_TOKEN = '</s>'
    tok_fun = lambda s: list(s) if level == "char" else s.split()

    """音频"""
    #audio_field = data.Field(sequential=True,
    #                         use_vocab=False,
    #                         dtype=torch.float32,
    #                         batch_first=True,
    #                         include_lengths=True,
    #                         pad_token=torch.ones((80,)) * TARGET_PAD, )
...
#
#             batch.src[i] = batch.src[i].lower()
#
#         return batch
#
#     if train:
#         # optionally shuffle and sort during training
#         train_data_iter, val_data_iter = data.BucketIterator.splits(
#             datasets=(dataset, val_dataset),
#             batch_size=batch_size,
#             sort_key=lambda x: len(x.src), shuffle=shuffle, device='cuda' if torch.cuda.is_available() else 'cpu')
#     else:
    ...
    def __init__(self, path, exts, fields, trg_size, skip_frames=1, **kwargs):
        ...
        with io.open(src_path, mode='r', encoding='utf-8') as src_file, \
            ...
            for src_line, trg_line, files_line in zip(src_file, trg_file, files_file):
                ...
                #audio_line = [float(joint) for joint in audio_line]
                ...
                #audio_frame = [audio_line[i:i + audio_size] for i in
                #               range(0, len(audio_line), audio_size * skip_frames)]
